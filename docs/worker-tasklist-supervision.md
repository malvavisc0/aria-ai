...
3. **`_settle_steps` signature** (§5.2) gains `exc: Exception | None = None`
   (the failure path needs it for the step `result`; §8.2 tests already pass
   it); crash-path safety re-scoped (planner tables guaranteed present
   because the web process seeded the plan before `Popen`, §7).
...
- The worker runs a `WorkerAgent` (`src/aria/agents/worker.py`) which **already has the `plan` tool** — registry `CORE` includes `reasoning, plan, scratchpad, shell` (`registry.py:102-128`), and `get_worker_agent` loads `[CORE, FILES, AX]` (`worker.py:60-62`).
...
- **Spawn paths.** `tools/worker/functions._spawn` (reached via `ax worker spawn`) is the only live spawn path. `src/aria/cli/worker/__init__.py` contains a duplicate Typer `spawn` command, but that app is **not registered** in `src/aria/cli/main.py` (no `aria worker` subcommand exists; the module is never imported from production code) — dead weight, left untouched by this feature. No second spawn path needs `steps`/`--plan-id` wiring.
...
User ──prompt──▶ Aria (web process)
                  ...
                  │   On exit: _settle_steps makes the DB terminal. │
                  │ └───────────────────────────────────────────────┘
                  │
                  ▼ ┌────────────── web process (async) ───────────┐
...
3. **The worker self-reports per step** via the `plan` tool it already has (`CORE` registry). The only new runner behavior is the Plan-section prompt injection and `_settle_steps` at exit.
...
2. **Polling (`watch_worker`, §5.3).** The watcher task iterates the async generator:
   ...
   - The generator terminates after yielding the **final** view once, when `worker_status` ∈ `{completed, failed, cancelled, zombie}`. No total poll budget: a worker may run for hours; liveness comes from the terminal status (audit JSON rewrite by `_record_completion`/`_record_failure`, or `is_process_running`→false for zombies).
...
- Orphaned CLI note: `src/aria/cli/worker/__init__.py` has its own `spawn` Typer command (its own copy of the cmd/audit logic) but is unregistered — not in `src/aria/cli/main.py`, never imported from production code, no `aria worker` subcommand. Left untouched: with no live entry point, it needs no `steps`/`--plan-id` changes, and the runner-only contract (`--plan-id` required) is fully enforced through the one live path.
...
- `_build_prompt` (`_runner.py:28-36`) **always** appends a **Plan section** to the prompt:
  ...
  - Finalize step statuses so the panel never lies. Add a module helper:
    ```python
    def _settle_steps(plan_id: str, mode: str, exc: Exception | None = None) -> None:
        ...
        """
    ```
  - On **completion** (`_record_completion`, `_runner.py:67-80`): mark remaining `pending`/`in_progress` steps as `completed` via `PlannerDatabase().update_step(plan_id, step_id, status="completed")` after loading the plan (`db.load_plan(plan_id)`, `database.py:77`). Iterate steps in `step_number` order (already how `load_plan` returns them, `database.py:89-101`).
  - On **failure** (`_record_failure`, `_runner.py:55-64`): mark the current `in_progress` step — **else the first `pending` step** if there is no `in_progress` (the worker may crash before it calls `plan(update, in_progress)` for the step it was working on) — as `failed` with `result=str(exc)` via `update_step(..., status="failed", result=str(exc))`. This is **best-effort attribution**: it fails one step so the panel shows a concrete failure point rather than a sea of `pending`, but it does not guarantee the failed step is the one that actually crashed. Do not attempt to detect "the real failing step" — there is no signal for it after a crash.
  - Extend `_record_completion`/`_record_failure` signatures with a `plan_id: str` param, now always provided by `_run`: `_record_completion(worker_id, result_text, result_file, tool_calls, args.plan_id)` at `_runner.py:137` and `_record_failure(worker_id, e, args.plan_id)` at `_runner.py:140`. `_settle_steps` runs **after** the audit write in both paths, so a poll never sees "audit terminal + unsettled steps" for long; the transient gap is covered by the render-only override (§6). Early-return inside `_settle_steps` if `db.load_plan(plan_id)` is `None` (defensive — the plan should always exist since spawn created it).
  - **Crash-path safety**: `_record_failure` runs inside the bare `except Exception as e:` at `_runner.py:139`, reached after an arbitrary workflow crash. `_settle_steps` is safe there: it uses only `PlannerDatabase()` (a fresh session per call on the singleton engine — the workflow holds no planner session to reuse), and the `plans`/`plan_steps` tables are guaranteed present in the shared `tools.db` because the web process ran `PlannerDatabase().save_plan` (hence `create_tables`) **before** `Popen` (§7). A crash before `get_worker_agent` just means step statuses were never updated — settle still reads the seeded plan and settles it. This reasoning is stated in the `_settle_steps` docstring (above) so an agent does not "fix" it by accepting a session arg.
- No changes to `WorkerAgent` — it already has the `plan` tool via `CORE` (`registry.py:108-112`).
...
class WorkerView:        # one renderable unit
    ...
    worker_status: str   # "running" | "completed" | "failed" | "cancelled" | "zombie"
...
async def watch_worker(worker_id: str, interval: float = 1.5
...
class WorkerTaskList:
    ...
    def __init__(self, worker_id: str, for_id: str | None = None,
...
- **Terminal-override render rule** (§6): when `view.worker_status` is terminal and ≠ `"completed"`, mark the first unfinished step (`in_progress`, else first `pending`) as `cl.TaskStatus.FAILED` **in the render only** — do not write to the DB (the runner's `_settle_steps` owns DB truth; this is a display safety net for zombies where the runner never settled).
...
- **Watcher state lives on `cl.user_session`, not `_state`, not a module dict.** Decided: watcher state is per-session (one user's running workers must not leak to another user's session). `cl.user_session` is session-scoped and is the existing home for per-session handles (`memory`, `thread_titled`, `_mcp_sessions` in `hooks.py`). A module-level `_watchers = {}` is banned by AGENTS.md ("no mutable module-level globals"); `_state` is app-wide (shared `db_engine`, `agents_workflow`), so it is the wrong scope. Use `cl.user_session.set("_supervision_watchers", …)`.
...
5. **Keep the completion-reasoning block (lines 47-57)** — "Did every step succeed? …" now reads the *handed-in* plan's statuses, which is exactly what `_settle_steps` guarantees are terminal.
...
| `failed` | `FAILED` | as-is |
| worker terminal ≠ completed (failed/cancelled/zombie) with unfinished steps | first unfinished step → `FAILED` | render-only override (DB untouched) |
...
- **Worker edits its own plan** (`plan(add/remove/reorder)`) → renderer rebuilds the task list on structural diff (step-id set/order/count change). `_settle_steps` reads the current plan at finalize, so it settles whatever steps exist then.
...
Fixture: `test_tools_db` for `_settle_steps`.
...
- `test_settle_steps_completed_flips_unfinished` — seed a plan with 1 `completed`, 1 `in_progress`, 2 `pending`; `_settle_steps(plan_id, "completed")` → all 4 become `completed` (load via `db.load_plan`).
- `test_settle_steps_failed_fails_in_progress_step` — seed 1 `in_progress`, 2 `pending`; `_settle_steps(plan_id, "failed", exc=ValueError("boom"))` → the `in_progress` step is `failed` with `result == "boom"`; the 2 `pending` stay `pending`.
- `test_settle_steps_failed_no_in_progress_fails_first_pending` — seed 3 `pending`, no `in_progress`; `_settle_steps("failed", exc=…)` → first `pending` is `failed`, rest `pending`. (Best-effort attribution — see §10 risk note.)
- `test_settle_steps_no_plan_returns_silently` — `_settle_steps("nonexistent-id", "completed")` returns without raising (no DB row).
...
- That `_settle_steps` iterates `step_number` order (that's the DB's `order_by`, not our logic — `load_plan` already guarantees it).
...
- **Worker ignores plan-tool instructions** (LLM behavior) → runner `_settle_steps` (§5.2) guarantees terminal coherence even if mid-run statuses were sloppy; the panel may jump READY→DONE but never lies at the end. The render-only terminal override (§6) covers the zombie case where the runner never settles.
- **Worker re-authors a plan instead of executing the seeded one** (the `worker.md` "Start with plan. Create…" conflict, §5.7) → the rewritten instruction + runner Plan section both say "execute the existing plan"; `_settle_steps` still settles whatever plan the worker ends up touching, so even a misbehaving worker cannot leave the panel incoherent. The `worker.md` regression test (§8) guards against the old wording returning.
