...
feature matrix (chat mode × GPU) lives in :mod:`aria.bootstrap`; mode and
choice resolution lives in :mod:`aria.cli.init_mode`; this command drives
them and prints a progress + summary report.
...
from aria.cli.init_mode import _build_choices, _resolve_chat_mode
...
def _add_tier_rows(table: Table, report) -> None:
...
def _voice_vision_note(interactive: bool, choices: FeatureChoices) -> str | None:
    """S5: make the off-by-default voice/vision discoverable in interactive
    mode (re-run with the opt-in flags to enable)."""
    if interactive and not choices.voice and not choices.vision:
        return "off (re-run with --with-voice/--with-vision to enable)"
    return None
...
def _print_summary(
    ...
    table.add_row("NVIDIA GPU", "yes" if report.hardware.has_nvidia_gpu else "no")
    _add_tier_rows(table, report)
    note = _voice_vision_note(interactive, choices)
    if note:
        table.add_row("Voice/vision", note)
    for label, values in (
        (".env changed", report.changed_env_keys),
        ("Installed", installed),
...
def _print_preflight_failures(out: Console, failures) -> None:
    for failure in failures:
        out.print(f"{failure.name}: {failure.error}")
        if failure.hint:
            out.print(f"[dim]  → {failure.hint}[/dim]")


def _verify_preflight(dry_run: bool) -> None:
    """Run preflight (step 8); exit 1 on a hard failure.

    Hardware-fit findings (KV-cache / memory budget) are advisories in the
    init context: init does not run ``aria config optimize`` (tier values
    are the defaults) and vLLM clamps the context at runtime
    (``_clamp_context_to_gpu_kv``), so a fresh GPU container must not be
    killed by them. Everything else init itself installs or writes
    (environment, binaries, models, storage, tools, …) still hard-fails.
    """
    if dry_run:
        return
    ...
    hardware_failures = [f for f in result.failures if f.category == "hardware"]
    hard_failures = [f for f in result.failures if f.category != "hardware"]

    if hardware_failures:
        console.print(
            "[yellow]⚠ Hardware-fit advisories (non-fatal — tune with "
            "`aria config optimize`; vLLM clamps context at runtime):[/yellow]"
        )
        _print_preflight_failures(console, hardware_failures)

    if hard_failures:
        _print_preflight_failures(error_console, hard_failures)
        raise typer.Exit(1)

    console.print("[green]✓[/green] Preflight passed")


def _bootstrap_aria_home() -> None:
    """Step 1 — env file, dirs, DB, logs, assets, chainlit config (idempotent)."""
...
def _apply_features(
    ...
    """Step 4 — write the feature matrix, then reload env from disk."""
...
def _finalize(report, resolved_mode: str, dry_run: bool) -> None:
    """Steps 7-9 tail — warning, marker, sanity check.

    The marker lands only after a fully successful init (Decision 3):
    ``run_init`` deferred it (``write_marker=False``) so install/download/
    preflight failures abort before the entry-point gate passes.
    """
    if dry_run:
        return
    if report.warning:
        console.print(f"[yellow]⚠ {report.warning}[/yellow]")
    write_init_completed_marker(resolved_mode, report.tier)
...
def init_command(
    ...
    resolved_mode, prompted = _resolve_chat_mode(mode, hw, non_interactive, dry_run)
    choices = _build_choices(
        resolved_mode,
        hw,
        non_interactive,
        prompted.remote_url or remote_url,
        prompted.remote_api_key or api_key,
        prompted.remote_model or model,
        with_voice,
        no_voice,
        with_vision,
        no_vision,
    ...
    report = _apply_features(resolved_mode, hw, choices, dry_run)
    ...
    # Step 8 — verify (preflight); hardware-fit findings are advisory only
    _verify_preflight(dry_run)

    # Step 9 — summary, then the completion marker on success only
    _print_summary(
        report,
        installed,
        downloaded,
        interactive=mode is None and not non_interactive and not dry_run,
        choices=choices,
    )
    _finalize(report, resolved_mode, dry_run)
