...
Each class body is also stored as source text in ``_ENV_CLASS_BODIES``
so :func:`aria.config.reload_env` can re-execute it and pick up
...
class Vllm:
    ...
    version: str = get_optional_env("ARIA_VLLM_VERSION", "0.28.0")
...
class Lightpanda:
    ...
    def is_available(cls) -> bool:
        """Check if Lightpanda is installed and ready to use.
...
def _extract_class_bodies() -> dict[str, str]:
    """Extract each config class body from this module's source.

    The bodies are stored so ``reload_env()`` can re-execute them
    against a fresh environment without re-importing the module.
    """
    import inspect
    import textwrap

    source = inspect.getsource(inspect.getmodule(Vllm) or __import__(__name__))
    bodies: dict[str, str] = {}
    for name in ("Vllm", "KnowledgeHub", "Lightpanda", "Voice"):
        marker = f"class {name}:"
        ...
        offset = 0
        for line in lines:
            # Body ends at the next top-level (column-0) statement.
            if line and not line[0].isspace() and line.strip():
                end = offset
                break
            offset += len(line)
        bodies[name] = textwrap.dedent(rest[:end])
    return bodies


_ENV_CLASS_BODIES: dict[str, str] = _extract_class_bodies()
